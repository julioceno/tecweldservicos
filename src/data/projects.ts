export const projects = [
  {
    id: "montagem-mecanica-processo-liquido",
    title: "Montagem Mecânica de Processo Líquido",
    image: "/projects/project-1/img-1.jpeg",
    folder: "/projects/project-1",
    paragraph:
      "Realizamos a montagem mecânica completa de sistemas de processo líquido, incluindo a instalação de tanques, tubulações de aço inox, bombas e válvulas. Este projeto envolveu a aplicação de soldagem TIG para garantir um acabamento sanitário, essencial para a indústria alimentícia. A Tecweld garantiu a execução do projeto com máxima segurança e dentro do cronograma.",
  },
  {
    id: "fabricacao-spool-inox-encamisada",
    title: "Fabricação de Spool Inox Encamisada",
    image: "/projects/project-2/img-1.jpeg",
    folder: "/projects/project-2",
    paragraph:
      "Desenvolvemos spools de tubulação em aço inox encamisada para sistemas que exigem alta resistência à corrosão e controle térmico. O processo incluiu o corte, dobra, soldagem e testes rigorosos para assegurar a qualidade do produto final. Nós da Tecweld utilizamos técnicas avançadas de soldagem para garantir a integridade das juntas, atendendo às especificações técnicas do cliente.",
  },
  {
    id: "instalacao-linha-tubulacao-industrial",
    title: "Instalação de Linha de Tubulação Industrial",
    image: "/projects/project-1/img-3.jpeg",
    folder: "/projects/project-1",
    paragraph:
      "Projeto de instalação de uma nova linha de tubulação para vapor e condensado em uma planta industrial. O serviço incluiu o pré-fabricação dos spools, a montagem em campo e a realização de testes de pressão. Utilizamos técnicas avançadas de soldagem para garantir a integridade e a durabilidade da tubulação.",
  },
];  